...
const CLOZE_BANKS = {
  middle: {
    ...
    key: 'middle',
    name: '中考七选五',
    description: '中考难度七选五阅读理解',
    masterFile: 'cloze_middle.json',
    statsFile: 'cloze_middle_stats.json',
    aliases: ['middle', '中考', '七选五中考'],
    categories: ['生活', '健康', '教育', '环保', '故事']
  },
  high: {
    id: '高考',
    key: 'high',
    name: '高考七选五',
    description: '高考难度七选五阅读理解',
    masterFile: 'cloze_high.json',
    statsFile: 'cloze_high_stats.json',
    aliases: ['high', '高考', '七选五高考'],
...
const getBankConfig = (bankId) => {
  if (CLOZE_BANKS[bankId]) return CLOZE_BANKS[bankId];
...
const getMasterQuestionsPath = (bankId = 'middle') => {
  ...
  return path.join(
    __dirname, 
    "../", 
    "resource/english_test_cloze_sentence_master",
    bank.masterFile
  );
};
...
/**
 * 获取七选五题目
 * GET /api/cloze/questions?type=random&bank=middle&count=1
 */
...
router.post("/submit", async (req, res) => {
  try {
    ...
    for (let i = 0; i < questionIds.length; i++) {
      ...
      // 确保用户答案是对象格式 { blank1: 'A', blank2: 'B', ... }
      if (typeof userAnswer === 'object' && userAnswer !== null) {
        // 检查每个空的答案是否正确
        const allCorrect = Object.entries(question.correctAnswers).every(([blankKey, correctOption]) => {
          return userAnswer[blankKey] === correctOption;
        });
        isCorrect = allCorrect;
      } else {
        console.log('答案格式错误，期望对象:', userAnswer);
        isCorrect = false;
      }

      // 模拟答题时间
      const responseTime = timeSpent ? Math.floor(timeSpent / questionIds.length) : Math.floor(Math.random() * 60) + 30;
...
/**
 * 获取题目详情（带解析）
 * GET /api/cloze/questions/:questionId/details?bank=middle
 */
...
/**
 * 获取学习报告
 * GET /api/cloze/report?bank=middle
 */
...
/**
 * 健康检查
 * GET /api/cloze/health
 */
router.get("/health", async (req, res) => {
  res.json({ flag: 1, message: "七选五 API is running" });
});
