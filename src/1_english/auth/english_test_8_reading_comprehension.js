...
const BANK_CONFIG = {
  ...
  masterFile: 'reading_comprehension_master.json',
...
const getMasterQuestionsPath = () => {
  return path.join(
    __dirname,
    "../",
    "resource/english_test_8_test_reading",  // 修改为新路径
...
router.get("/info", async (req, res) => {
  try {
    ...
    console.log('阅读理解题库路径:', masterPath);
...
router.get("/passages", async (req, res) => {
    try {
      ...
      // ✅ 修改这里：返回完整的篇章数据，包含 content 和 questions
      const passages = masterData.passages?.map(p => ({
        ...
        content: p.content,           // 👈 添加文章内容
        questions: p.questions,        // 👈 添加题目数据
        givenWords: p.givenWords || [], // 👈 添加所给词
...
/**
 * 获取阅读理解篇章
 * GET /api/reading/passage?passageId=xxx&type=random
 */
...
/**
 * 提交阅读理解答案
 * POST /api/reading/passage/submit
 */
...
/**
 * 获取篇章详情（带解析）
 * GET /api/reading/passage/:passageId/details
 */
