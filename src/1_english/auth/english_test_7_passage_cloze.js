...
const BANK_CONFIG = {
  id: '篇章完形填空',
  name: '篇章完形填空练习',
  description: '根据上下文选择正确的词并用适当形式填空',
  masterFile: 'passage_cloze_master.json',
  statsFile: 'passage_cloze_stats.json',
  categories: ['动物', '生活', '故事', '科普', '健康']
};
...
const getMasterQuestionsPath = () => {
  return path.join(
    __dirname,
    "../",
    "resource/english_test_7_passage_cloze",  // 修改这里
    BANK_CONFIG.masterFile
  );
};
...
const getStudentStatsPath = (username) => {
  return path.join(
    ...
    "person_name",
    username,
    "test_7_passage_cloze",  // 统计目录可以保持不变或也修改为 test_7_passage_cloze
...
/**
 * 获取题库信息
 * GET /api/7_passage_cloze/info  // 可以修改API路径，也可以保持原样
 */
...
/**
 * 获取所有篇章列表
 * GET /api/7_passage_cloze/passages
 */
...
/**
 * 获取篇章题目
 * GET /api/7_passage_cloze/passage?passageId=xxx&type=random
 */
...
/**
 * 提交篇章答案
 * POST /api/7_passage_cloze/passage/submit
 */
...
/**
 * 获取篇章详情（带解析）
 * GET /api/7_passage_cloze/passage/:passageId/details
 */
...
/**
 * 获取学生统计报告
 * GET /api/7_passage_cloze/report
 */
...
/**
 * 健康检查
 * GET /api/7_passage_cloze/health
 */
router.get("/health", async (req, res) => {
  res.json({ 
    flag: 1, 
    message: "篇章完形填空模块运行正常",
    config: {
      bank: BANK_CONFIG,
